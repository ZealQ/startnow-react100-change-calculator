...
describe('express', function() {
  ...
  it('should include an input element for the user to enter amount due', () =>
    nightmare
    .goto(url)
    .evaluate(() => document.querySelector('input[id=amountDue]'))
    .end()
    .then((input) => {
  ...
  it('should include an input element for the user to enter amount received', () =>
    nightmare
    .goto(url)
    .evaluate(() => document.querySelector('input[id=amountReceived]'))
    .end()
    .then((input) => {
      expect(input).to.not.be.null;
      expect(typeof input).to.equal('object');
  ...
  it('should calculate individual change correctly', () =>
    ...
    .click('button.btn')
    .wait('div.alert.alert-success')
    .evaluate(() => Array.from(document.querySelectorAll('div > p')).map(e => e.innerText))
    .then((results) => {
      const expected = {
        twenties: '0',
        tens: '0',
